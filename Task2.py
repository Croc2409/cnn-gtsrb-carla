...
# Creating a class to load and transform the test dataset
...
class ResNet18(nn.Module):
    def __init__(self, n_image_channel):
        ...
        self.cnn_layer = nn.Sequential(
            nn.Conv2d(in_channels=n_image_channel, out_channels=64, kernel_size=7, stride=2, padding=3),
            nn.BatchNorm2d(64),
            nn.ReLU(),
        ...
        self.neuron_layer = nn.Sequential(
            nn.Linear(512, 43)
        )

    def forward(self, X):
        output = self.cnn_layer(X)
        final_output = self.neuron_layer(output)
        return final_output


# Using SGD+Momentum as the optimizer
model = ResNet18(n_image_channel=3).to(device)
